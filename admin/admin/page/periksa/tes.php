...
$sqlklinik="select ID_KLINIK , NAMA_KLINIK from tb_klinik  order by NAMA_KLINIK asc ";
$resklinik   = mysqli_query($conn,$sqlklinik);
$check = mysqli_num_rows($resklinik);
?>
<!DOCTYPE html>
<html lang="en">
<head>
	...
	<script type="text/javascript">
		function pilihklinik(ID_KLINIK){
			if(ID_KLINIK!="-1"){
				loadData('klinik',ID_KLINIK);
				$("#kecamatan_dropdown").html("<option value='-1'>Pilih KLINIK</option>");	
			}else{
		...
		function pilihdokter(ID_DOKTER){
			if(ID_DOKTER!="-1"){
				loadData('dokter',ID_DOKTER);
			}else{
				$("#kecamatan_dropdown").html("<option value='-1'>Pilih dokter</option>");		
			}
		}
